...
import com.techhounds.houndvision.filters.BlackWhite;
import com.techhounds.houndvision.filters.ColorRange;
import com.techhounds.houndvision.filters.ColorSpace;
import com.techhounds.houndvision.filters.Dilate;
import com.techhounds.houndvision.filters.Erode;
import com.techhounds.houndvision.filters.GrayScale;
import com.techhounds.houndvision.filters.MatFilter;
import com.techhounds.houndvision.utils.DrawTool;
import com.techhounds.houndvision.utils.FovCalculator;
import com.techhounds.houndvision.utils.PolyArrayRender;
import com.techhounds.houndvision.utils.PolygonRender;
import com.techhounds.houndvision.utils.RectangleRender;
import com.techhounds.houndvision.utils.RotationEstimator;
import com.techhounds.houndvision.utils.ScalarColors;
import com.techhounds.houndvision.filters.CrossHair;
import com.techhounds.houndvision.utils.*;
...
*/
    	...
    	if(offsetXDegrees < (Camera.FOV_X_DEGREES/2) && offsetXDegrees > (-Camera.FOV_X_DEGREES/2) && printToNetWorkTable) {
  			final double offset = -Math.toDegrees(distanceOffset(foundTarget.getCenterX(), foundTarget.getCenterY()));

			DataIO.getInstance().send("offset::" + offset);

			LiveView.getLiveView().runOnUiThread(new Runnable() {
				@Override
				public void run() {
					((TextView) LiveView.getLiveView().findViewById(R.id.offsetDegrees)).setText("Offset: " + offset);
				}
			});
    	}
    	...
    	if(printToNetWorkTable) {
			final double distToBase = baseDistanceInches;
			final double distToTarget = targetDistanceInches;

			DataIO.getInstance().send("distToBase::" + distToBase);
			DataIO.getInstance().send("distToTarget::" + distToTarget);

			LiveView.getLiveView().runOnUiThread(new Runnable() {
				@Override
				public void run() {
					((TextView) LiveView.getLiveView().findViewById(R.id.distToBase)).setText("Dist To Base: " + distToBase);
					((TextView) LiveView.getLiveView().findViewById(R.id.distToGoal)).setText("Dist To Goal: " + distToTarget);
				}
			});
		}
